...
printMessage "Project: ${project_name}   Part: ${project_part}"
...
#------------------------------------------------------------------------
printMessage "Include HDL files into project"
...
set files [list \
 {% for tb_data_file in tb_data_file_list %}"[file normalize "{{ tb_data_file["file"] }}"]" \
 {% endfor %}]
add_files -norecurse -fileset $obj $files

{% for tb_data_file in tb_data_file_list %}
set file "{{ tb_data_file['file'] }}"
set file [file normalize $file]
set file_obj [get_files -of_objects [get_filesets sim_1] [list "*$file"]]
set_property -name "file_type" -value "{{ tb_data_file['file_type'] }}" -objects $file_obj
{% if tb_data_file.get('library') is not none %}set_property -name "library" -value "{{ tb_data_file['library'] }}" -objects $file_obj
{% endif %}{% endfor %}

set obj [get_filesets sim_1]
set_property "top" "{{ tb_top_entity }}" $obj
set_property -name "top_auto_set" -value "0" -objects $obj
set_property -name "top_file" -value "{{ tb_top_entity_file }}" -objects $obj
